...
piano['tones'] = ['c', 'c+', 'd', 'd+', 'e', 'f', 'f+', 'g', 'g+', 'a', 'a+', 'b']
...
def init():
    global serial, surface
    # serial = Serial(port=get_port(), baudrate=get_baudrate())

    pygame.init()
    pygame.display.set_caption('Piano')
    surface = pygame.display.set_mode((WINDOW_WIDTH, WINDOW_HEIGHT), pygame.SCALED)
...
def press(idx: int):
    global piano
    piano['pressed'][idx] = not piano['pressed'][idx]
